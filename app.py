...
@st.cache_resource
def load_llms():
    """Loads two independent LLMs. Each one predicts AND explains on its own,
    with no input from the trained classifiers."""
    llm1_name = "Qwen/Qwen2.5-1.5B-Instruct"
    llm2_name = "Qwen/Qwen2.5-0.5B-Instruct"

    llm1_tok = AutoTokenizer.from_pretrained(llm1_name)
    llm1_mdl = AutoModelForCausalLM.from_pretrained(llm1_name, dtype=torch.float32)

    llm2_tok = AutoTokenizer.from_pretrained(llm2_name)
    llm2_mdl = AutoModelForCausalLM.from_pretrained(llm2_name, dtype=torch.float32)

    return llm1_tok, llm1_mdl, llm2_tok, llm2_mdl


def llm_predict_and_explain(tok, mdl, text):
    """A single LLM independently predicts Human/AI and explains its own reasoning.
    No classifier output is given to the model; it only sees the raw text."""
    prompt = f"""Read this text and decide if it was written by a human or by AI.

Respond in exactly this format:
Verdict: Human
or
Verdict: AI
Reason: (2-3 sentences explaining why, based on the text's style, vocabulary, and structure)
...
"""
    ...
    outputs = mdl.generate(
        **inputs,
        max_new_tokens=150,
        do_sample=False  # greedy decoding for reliable, consistent verdicts
    )
    response = tok.decode(outputs[0][inputs['input_ids'].shape[1]:], skip_special_tokens=True)
    return response.strip()


def parse_verdict(llm_text):
    """Extracts Human/AI/Unclear from an LLM's raw response."""
    verdict_section = llm_text.split("Reason:")[0]
    if "AI" in verdict_section:
        return "AI"
    elif "Human" in verdict_section:
        return "Human"
    return "Unclear"
...
with st.sidebar:
    ...
    selected_model = st.selectbox(
        "Choose a classifier for primary prediction",
        ["FNN", "SVM", "AdaBoost", "LSTM", "CNN", "Decision Tree"],
    )
    st.markdown("---")
    st.markdown(
        "This app detects whether text was written by a human or generated by AI. "
        "It uses six trained classifiers, plus two independent LLMs (Qwen2.5-1.5B and "
        "Qwen2.5-0.5B) that each make their own prediction and explain their reasoning."
    )
    st.markdown("---")
...
if analyze and text_input:
    with st.spinner("Running classifier predictions..."):
    ...
    # Precompute shared values needed by later sections
    ...
    st.markdown("---")
    st.markdown("## All Classifier Predictions")
    ...
    with col2:
        fig2 = px.bar(
            ...
            title='AI Probability Score by Classifier',
            range_y=[0, 1],
        )
    ...
    # ── LLM Analysis: two independent LLMs, each predicts and explains ──────────
    st.markdown("---")
    st.markdown("## LLM Predictions")
    st.markdown(
        "Each LLM below reads only the raw text. Neither one is told what the "
        "classifiers predicted. Each makes its own independent call and explains why."
    )

    with st.spinner("Running independent LLM predictions..."):
        llm1_tok, llm1_mdl, llm2_tok, llm2_mdl = load_llms()

        llm1_output = llm_predict_and_explain(llm1_tok, llm1_mdl, text_input)
        llm2_output = llm_predict_and_explain(llm2_tok, llm2_mdl, text_input)

        llm1_verdict = parse_verdict(llm1_output)
        llm2_verdict = parse_verdict(llm2_output)

    col1, col2 = st.columns(2)

    with col1:
        st.markdown("#### Qwen2.5-1.5B-Instruct")
        st.info(llm1_output)

    with col2:
        st.markdown("#### Qwen2.5-0.5B-Instruct")
        st.info(llm2_output)

    st.markdown("#### Verdict Comparison")
    classifier_verdict = "AI" if primary['prediction'] == 1 else "Human"

    summary_df = pd.DataFrame([
        {"Source": f"Classifier ({selected_model})", "Verdict": classifier_verdict},
        {"Source": "Qwen2.5-1.5B-Instruct",          "Verdict": llm1_verdict},
        {"Source": "Qwen2.5-0.5B-Instruct",          "Verdict": llm2_verdict},
    ])
    st.dataframe(summary_df, use_container_width=True, hide_index=True)

    verdicts = [classifier_verdict, llm1_verdict, llm2_verdict]
    if len(set(verdicts)) == 1:
        st.success(f"All three sources agree: {classifier_verdict}")
    else:
        st.warning("The classifier and the two LLMs do not all agree, see the table above.")
    ...
    st.markdown("Top TF-IDF terms from your input text that influenced the classifier prediction.")
    ...
    report_lines = [
        ...
        "=" * 40,
        f"Primary Classifier : {selected_model}",
        f"Prediction          : {label}",
        f"Confidence          : {conf:.1%}",
        "",
    ...
    report_lines += [
        "",
        "Qwen2.5-1.5B-Instruct (independent prediction + explanation):",
        "-" * 40,
        llm1_output,
        "",
        "Qwen2.5-0.5B-Instruct (independent prediction + explanation):",
        "-" * 40,
        llm2_output,
        "",
        "Verdict Comparison:",
        "-" * 40,
        f"Classifier ({selected_model}) : {classifier_verdict}",
        f"Qwen2.5-1.5B-Instruct        : {llm1_verdict}",
        f"Qwen2.5-0.5B-Instruct        : {llm2_verdict}",
        "",
        "Text Statistics:",
        "-" * 40,
